...
//			}
            ...
            sprintf(output,"%2.2f%cV          ", calcVal,prefix);			//2.2 allows 2 places either side of dp
		...
		if(modeSelect == BTN2) // if current button pressed
	{
		...
		while(buttonPressed != BTN8){ //checks back button has not been pressed
			...
			snprintf(output,16,"%2.2f%cA            ", calcVal,prefix); //2.2 allows 2 places either side of dp
			LCD_PutS(output); //Outputs to LCD
